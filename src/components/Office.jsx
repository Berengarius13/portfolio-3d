...
// Use motion control and change scaling of objects as section changes 
// Change individual behaviour of each object that you want to scale in the scene
export function Office(props) {
  ...
  texture.flipY = false;
  // Texture is pre baked in rendered, now loaded directly here
  ...
  // These motion values are used to control the opacity of textures in an animation.
  const textureOpacity = useMotionValue(0);
  const glassTextureOpacity = useMotionValue(0);

  // When section value is changed animation is triggered
  // animate function is used to animate the motion values
  // animate(motion value we want to animate, target value we want to animate towards)
  // It is changing opacity of texture as the section is changing
  useEffect(() => {
    // function used to manually start and control animation
  ...
  // Just before frame is going to be rendered use frame will update the opacity values
  useFrame(() => {
...
// Technique is to load them before needed to improve the user experience
